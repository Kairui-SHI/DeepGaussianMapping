...
def plot_global_pose(location, output_dir, Init_location=None, gt_location=None, epoch=None):
    ...
    ax.quiver(location[:, 0], location[:, 1], u, v, t, scale=20, scale_units='inches', width=1e-3, cmap='Greens')
    if Init_location is not None:
        init_u = np.cos(Init_location[:, -1]) * 2
        init_v = np.sin(Init_location[:, -1]) * 2
        ax.quiver(Init_location[:, 0], Init_location[:, 1], init_u, init_v, t, scale=20, scale_units='inches', width=1e-3, cmap='Blues')
    if gt_location is not None:
        gt_u = np.cos(gt_location[:, -1]) * 2
        gt_v = np.sin(gt_location[:, -1]) * 2
        ax.quiver(gt_location[:, 0], gt_location[:, 1], gt_u, gt_v, t, scale=20, scale_units='inches', width=1e-3, cmap='Reds')
    ax.axis('equal')
    ax.tick_params(axis='both', labelsize=18)
    norm = colors.Normalize(0, location.shape[0])
    cbar = fig.colorbar(cm.ScalarMappable(norm=norm, cmap='Greens'), ax=ax)
    ...
    fig, ax = plt.subplots()
    fig.set_size_inches(15, 10)
    ax.quiver(location[:, 0], location[:, 2], u, v, t, scale=20, scale_units='inches', width=1e-3, cmap='Greens')
    if Init_location is not None:
        init_u = np.cos(Init_location[:, 4]) * 2
        init_v = np.sin(Init_location[:, 4]) * 2
        ax.quiver(Init_location[:, 0], Init_location[:, 2], init_u, init_v, t, scale=20, scale_units='inches', width=1e-3, cmap='Blues')
    if gt_location is not None:
        gt_u = np.cos(gt_location[:, 4]) * 2
        gt_v = np.sin(gt_location[:, 4]) * 2
        ax.quiver(gt_location[:, 0], gt_location[:, 2], gt_u, gt_v, t, scale=20, scale_units='inches', width=1e-3, cmap='Reds')
    ax.axis('equal')
    ax.tick_params(axis='both', labelsize=18)
    norm = colors.Normalize(0, location.shape[0])
    cbar = fig.colorbar(cm.ScalarMappable(norm=norm, cmap='Greens'), ax=ax)
